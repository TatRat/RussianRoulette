...
namespace TatRat.Editor
{
    ...
    {
        private const float NUMBER_COLUMN_WIDTH = 36f;
        
        protected SerializedProperty _arrayProperty;

        protected static float SingleLineHeight
            => EditorGUIUtility.singleLineHeight;

        protected static GUIContent PlusIcon =>
            EditorGUIUtility.IconContent(EditorGUIUtility.isProSkin ? "d_Toolbar Plus" : "Toolbar Plus");

        protected static GUIContent MinusIcon =>
            EditorGUIUtility.IconContent(EditorGUIUtility.isProSkin ? "d_Toolbar Minus" : "Toolbar Minus");

        public SerializedArrayTableView(SerializedProperty arrayProperty) : base(new TreeViewState(),
            CreateNewHeader(arrayProperty.Copy()))
        {
        ...
        private static MultiColumnHeader CreateNewHeader(SerializedProperty arrayProperty)
        {
            ...
            columns.Insert(0, new MultiColumnHeaderState.Column()
            {
                ...
                minWidth = NUMBER_COLUMN_WIDTH,
                width = NUMBER_COLUMN_WIDTH,
                maxWidth = NUMBER_COLUMN_WIDTH,
            });
